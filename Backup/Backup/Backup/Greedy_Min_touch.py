...
Loop_green_edges = []
for g in green_edges_1:
    Loop_green_edges.append(g)
#The loop implements the greedy Min-touch algorithm that iteratively adds new paths to the slution 
New_demands =green_edges_2
print 'Length New_demands:'
print len(New_demands)
"""
for g in green_edges:
...
print demand_flows_1
for h in demand_flows_1:
    Old_demand_flows.append(h)
    Deltah[h] = 0
    Thetah[h] = 0
##[OBJ, Deltah, Thetah, H2, my_used_arcs] = optimal_SDN(H,green_edges_1, K, demand_flows_1, w_l, w_h)
for g in New_demands:
    x=len(Loop_green_edges)
    print 'x:'
    print x
    name_flow='F%d'%(x)
    New_demand_flows.append(name_flow)
    Loop_green_edges.append(g)
    print 'Length green edges:'
    ...
    for h in Old_demand_flows:#New_demand_flows:#New_demand_flows:#range(1,len(New_demand_flows)+1):
        print 'h:'
        print h
        print 'Old demand flows:'
        print Old_demand_flows
        print 'New_Deltah:'
        print New_Deltah, New_Deltah[h], Deltah
        print 'New Thetah:'
        print New_Thetah, New_Thetah[h], Thetah
        #if New_Deltah[h] == 1:
        #    Deltah[h] = 1
        #elif New_Thetah[h] == 1:
        #    Thetah[h] =1
        if Deltah[h] ==1:
            New_Deltah[h] = 1
            New_Thetah[h] = 0
        elif (Thetah[h] ==1) and (New_Deltah[h]==0):
            New_Thetah[h] =1
            New_Deltah[h] =0
    Deltah=New_Deltah
    Thetah=New_Thetah
    Old_demand_flows =[]
    for h in New_demand_flows:
        Old_demand_flows.append(h)
    #New_demand_flows.remove(name_flow)
    #i+=1
    if my_used_arcs != []:
        for h in New_demand_flows:#demand_flows_1:
            for i,j in H.edges():
                K[h,i,j] = 0
                if (my_used_arcs[h,i,j] ==1):
                    K[h,i,j] = 1
...
print 'Thetah'
print Thetah
for h in demand_flows:#range(0, len(demand_flows)):#demand_flows:
