...
def my_draw(H,file_name):
    ...
    #path='c:\\Users\\user\\Seamus_Last\\LabWork\\image_graph_dot\\DotFile\\' + file_name
    ...
    while(line!=''):
        ...
        if flag_alt==0:
            if line!='stop\n':
                ...
                H.node[id_node]['true_status']='destroyed'
                H.node[id_node]['color']='red'
                if id_node not in nodes_destroyed:
                    nodes_destroyed.append(id_node)
            else:
                flag_alt=1
        else:
            #if line !='stop\n':
                ...
                for k in keydict:
                    if H[id_source][id_target][k]['type']=='normal':
                        H.add_edge(id_source,id_target,key=k, status='destroyed',true_status='destroyed', labelfont='red',color='red',style='dashed')
                        edges_destroyed.append((id_source,id_target))

        line=file.readline()
    return nodes_destroyed,edges_destroyed

def destroy_graph_manual_gray(H,file_path):

    for node in H.nodes():
        if H.node[node]['type'] == 'normal':
            H.node[node]['status'] = 'destroyed'
            H.node[node]['color'] = 'gray'
    ...
    while(line!=''):
        ...
        if flag_alt==0:
            if line!='stop\n':
                ...
                for k in keydict:
                    if H[id_source][id_target][k]['type']=='normal':
                        H.add_edge(id_source,id_target,key=k, true_status='destroyed')
                        edges_destroyed.append((id_source,id_target))
...
def destroy_graph_gray(H,mu_x,mu_y,sigma,error,seed_random):

    #random.seed(seed_random)

    nodes_dest,nodes_really_dest=destroy_nodes_gray(H,mu_x,mu_y,sigma,error,seed_random)
    edges_dest,edges_really_dest=destroy_edges_gray(H,mu_x,mu_y,sigma,error,seed_random)

    return nodes_dest,nodes_really_dest,edges_dest,edges_really_dest

def destroy_graph_gray_real(H,mu_x,mu_y,sigma,error,seed_random):

    #random.seed(seed_random)

    nodes_dest,nodes_really_dest=destroy_nodes_gray_real(H,mu_x,mu_y,sigma,error,seed_random)
    edges_dest,edges_really_dest=destroy_edges_gray_real(H,mu_x,mu_y,sigma,error,seed_random)

    return nodes_dest,nodes_really_dest,edges_dest,edges_really_dest

def destroy_random_graph_gray(H,prob,error):

    nodes_dest,nodes_really_dest=destroy_random_nodes_gray(H,prob,error)
    edges_dest,edges_really_dest=destroy_random_edges_gray(H,prob,error)
...
def destroy_nodes_gray(H,mu_x,mu_y,sigma,error,seed_random):
    ...
    for i in H.nodes():
        ...
        #print 'pdf calcolata: ' + str(pdf)
        prob=get_prob(pdf,x,y,sigma)
        #pdf_2=bivariate_normal((x+random.uniform(-error,error)),(y+random.uniform(-error,error)),sigma,sigma,mux=(mu_x),muy=(mu_y),sigmaxy=0)
        pdf_2=bivariate_normal(x,y,sigma,sigma,mux=(mu_x),muy=(mu_y),sigmaxy=0)
        est_prob= get_prob(pdf_2,x,y,sigma+error)
        #if (prob >= 0.5):
        #  est_prob = prob - error
        #else:
        #  est_prob = prob +error
        #est_prob=prob+random.uniform(-error,error) #get_prob(pdf_2,x,y,sigma)
        #print 'HHHHHHHHHHHHHHHHHHHH******************HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH'
        #print str(prob)
        #random.seed(seed_random)
        seed_random=int(seed_random)+1        
        if flip_coin_2(prob,seed_random):
            H.node[i]['true_status']='destroyed'
            if id_node not in nodes_really_dest:
                nodes_really_dest.append(id_node)
                #est_prob=prob+random.uniform(-error,error)
                if (est_prob <= 0): 
                  est_prob= 0.01
                if (est_prob > 1):
                  est_prob= 1.0
                H.node[i]['prob']=est_prob
            ...
            if (est_prob > 1):
              est_prob= 1.0
            H.node[i]['prob']=est_prob#+random.uniform(-error,error)
        ...
        if id_node not in destroyed_nodes:
            ...
            if (est_prob > 1):
              est_prob= 1.0
            H.node[i]['prob']=est_prob #prob+random.uniform(-error,error)
            #print 'nodo distrutto:' +str(i)
    print 'DEMIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF'
    print int(seed_random)
    return destroyed_nodes,nodes_really_dest

def destroy_nodes_gray_real(H,mu_x,mu_y,sigma,error,seed_random):
    ...
    for i in H.nodes():
        ...
        if flip_coin_2(prob,seed_random):
            ...
            if id_node not in destroyed_nodes:
                ...
                if (est_prob > 1):
                  est_prob= 1.0
                H.node[i]['prob']=1 #prob+random.uniform(-error,error)
            ...
            if (est_prob > 1):
              est_prob= 1.0
            H.node[i]['prob']=0#+random.uniform(-error,error)
...
def destroy_random_nodes_gray(H,prob,error):
...
def destroy_edges_gray(H,mu_x,mu_y,sigma,error,seed_random):
    ...
    for i in H.nodes():
        for j in H.nodes():
            ...
            if H.has_edge(id_source,id_target):
                ...
                #print 'pdf calcolata: ' + str(pdf)
                prob_arc=get_prob(pdf_arc,x_m,y_m,sigma)
                #pdf_2=bivariate_normal((x_m + random.uniform(-error,error)),(y_m+random.uniform(-error,error)),sigma,sigma,mux=(mu_x),muy=(mu_y),sigmaxy=0)
                #est_prob=get_prob(pdf_2,x_m,y_m,sigma)
                pdf_2=bivariate_normal(x_m,y_m,sigma,sigma,mux=(mu_x),muy=(mu_y),sigmaxy=0)
                est_prob= get_prob(pdf_2,x_m,y_m,sigma+error)
                #print 'prob_arco: ' + str(prob_arc)
                #random.seed(seed_random)
                #if (prob_arc >= 0.5):
                #  est_prob = prob_arc - error
                #else:
                #  est_prob = prob_arc +error
                #est_prob=prob_arc+random.uniform(-error,error)
                keydict =H[id_source][id_target]
                #print str(keydict)
                key=len(keydict)
                for k in keydict:
                    if 'normal' == H.edge[id_source][id_target][k]['type']:
                        ...
                        edge_reverse=(id_target,id_source)
                        seed_random=int(seed_random)+1	
                        if(flip_coin_2(prob_arc,seed_random)):
                            H[id_source][id_target][k]['status'] = 'destroyed'
                            ...
                            if (est_prob > 1):
                              est_prob= 1.0
                            H[id_source][id_target][k]['prob'] =est_prob #prob_arc+random.uniform(-error,error)
...
def destroy_edges_gray_real(H,mu_x,mu_y,sigma,error,seed_random):
                ...
                for k in keydict:
                    if 'green'!= H.edge[id_source][id_target][k]['type']:
                        ...
                        if(flip_coin_2(prob_arc,seed_random)):
                            H[id_source][id_target][k]['status'] = 'destroyed'
                            H[id_source][id_target][k]['true_status'] = 'destroyed'
                            H[id_source][id_target][k]['color'] = 'red'
                            H[id_source][id_target][k]['style'] = 'dashed'
                            #est_prob=prob_arc+random.uniform(-error,error)
                            H[id_source][id_target][k]['prob'] = 1.0 #prob_arc+random.uniform(-error,error)
                            ...
                            #est_prob=prob_arc+random.uniform(-error,error)
                            H[id_source][id_target][k]['prob'] = 0.0 #prob_arc+random.uniform(-error,error)
...
def destroy_random_edges_gray(H,prob,error):
...
#DIMAN ADDED recover_gray_Diman
def recover_gray_Diman(H,nodes_recovered_isp,edges_recovered_isp,nodes_really_dest, edges_really_dest,discovered_edges,discovered_nodes):
    for node in nodes_recovered_isp:
        if node not in nodes_really_dest:
            H.node[node]['status']='repaired'
            H.node[node]['color']='""'
        elif node in nodes_really_dest:
            H.node[node]['status']='repaired'
    ...
    for node in discovered_nodes:
        if node not in nodes_recovered_isp:
            ...
            elif node not in nodes_really_dest:
                if H.node[node]['color']!='green':
                    H.node[node]['status']='on'
                    H.node[node]['color']='""'	
				
	
# Add recovered edges to the graph			
    for edge in edges_recovered_isp:
        ...
        edge=(id_source,id_target)
        edge_reverse=(id_target,id_source)	
        if edge in edges_really_dest or edge_reverse in edges_really_dest:
            for k in keydict:
        ...
        elif edge not in edges_really_dest and edge_reverse not in edges_really_dest:
            #id_source=edge[0]
            #id_target=edge[1]
            #keydict =H[id_source][id_target]
            #print str(keydict)
            for k in keydict:
                if H.edge[id_source][id_target][k]['type']=='normal':
                    H.add_edge(id_source,id_target,key=k, status='repaired',labelfont='black',color='black',style='solid')
                    print 'arco ripristinato %d-%d: '%(id_source,id_target)
            if H.node[id_source]['true_status']=='destroyed':
                H.node[id_source]['status']='repaired'
                H.node[id_source]['color']='blue'
            ...
            if H.node[id_target]['true_status']=='destroyed':
                ...
                H.node[id_target]['color']='blue'
                print 'nodo ripristinato %d : '%(id_target)		
#Add Dicovered edges to the H graph			
        ...
        if edge not in edges_recovered_isp and edge_reverse not in edges_recovered_isp:
            ...
            if edge in edges_really_dest or edge_reverse in edges_really_dest:
                for k in keydict:
                    ...
                    #if H.edge[id_source][id_target][k]['type']=='destroyed':
                        #H.add_edge(id_source,id_target,key=k, status='destroyed',labelfont='red',color='red',style='solid')
                        #print 'arco ripristinato %d-%d: '%(id_source,id_target)
            elif edge not in edges_really_dest and edge_reverse not in edges_really_dest:
                #id_source=edge[0]
                #id_target=edge[1]
...
def write_stat_monitors(path_to_stats,filename_stat,seed_random,alfa,
                          ...
                          Num_Identified_links_pr, Num_Best_greedy_monitors_pr, # Greedy algorithm for preserving identifiability: The identifiable links, Selected number of monitors,
                          ...
                          Alg1MaxRank, Alg1MaxRankMonitors, Alg1RMaxRank, Alg1CostMaxRank, Alg1ProbeCost, Alg1MaxRank_Identi_link,  #Matrix maximum rank is, number of monitors for the max rank, Minimum number of probes, Cost of probes for the max rank, Limit on the probe cost
        ...
        if not os.path.exists(path_to_file_stat):
            #print 'non esiste lo creo'
            file=open(path_to_file_stat,'w+')
            name_of_colunms="Seed\tAlfa\tNumber_of_Random_Monitors\tTotal_Identifiable_Links\tMatrixRank\tTotalCost\tMonitor_Limit\tBrute_Force_Max_Identified_Links\tBrute_Force_Monitors\tGreedy_Identified_Links\tGreedy_Cost\tGreedy_Rank_Indentified_Links\tGreedy_Rank_Cost\tILP_identified_Links\tILP_Monitors\tLP_relaxation_identified_Links\tLP_relaxation_Monitors\tMinProbMonitors\tRMin\tCost\tIdentifiable_Links_MinProb\tMinimumMonitors\tMinProbLen\tCostMinMon\tIdentifiable_Links_MinProb_MinMon\tMaxRank_Alg2\tMaxRankMonitors_Alg2\tMaxRankProbNum_Alg2\tMaxRankProbCost_Alg2\tMaxRankLimitCost_Alg2\tMaxRnak_Identifiable_Links_Alg2\tMaxRank_Alg1\tMaxRankMonitors_Alg1\tMaxRankProbNum_Alg1\tMaxRankProbCost_Alg1\tMaxRankLimitCost_Alg1\tMaxRank_IdentifiableLinks_Alg1\tRank_of_MinMonMaxRank\tNumMon_MinMonMaxRank\tNumProbes_MinMonMaxRank\tIdentifiable_Links_MinMon_MaxRank\tNumberofNodes\tNumberofEdges\n"
        ...
        file=open(path_to_file_stat,'a')
        raw_line=str(seed_random)+'\t\t'+str(alfa)+'\t\t'+str(my_monitors)+'\t\t'+str(Num_Identi_link)+'\t\t'+str(MatrixRank)+'\t\t'+str(TotalCost)+'\t\t'+str(Monitors)+'\t\t'+str(Num_Selected_Links)+'\t\t'+str(Num_Selected_Monitors)+'\t\t'+str(Num_Identified_links)+'\t\t'+str(Num_Best_greedy_monitors)+'\t\t'+str(Num_Identified_links_pr)+'\t\t'+str(Num_Best_greedy_monitors_pr)+'\t\t'+str(Num_ILP_identifiable_links)+'\t\t'+str(Num_Best_ILP_monitors)+'\t\t'+str(Num_LP_relaxation_identifiable_links)+'\t\t'+str(Num_Best_LP_relaxation_monitors)+'\t\t'+str(MinProbMonitors)+'\t\t'+str(RMin)+'\t\t'+str(Cost)+'\t\t'+str(MinProbe_Identi_link)+'\t\t'+str(MinimumMonitors)+'\t\t'+str(MinProbeLen)+'\t\t'+str(CostMinMon)+'\t\t'+str(MinProb_MinMon_Identi_link)+'\t\t'+str(MaxRank)+'\t\t'+str(MaxRankMonitors)+'\t\t'+str(RMaxRank)+'\t\t'+str(CostMaxRank)+'\t\t'+str(ProbeCost)+'\t\t'+str(MaxRank_Identi_link)+'\t\t'+str(Alg1MaxRank)+'\t\t'+str(Alg1MaxRankMonitors)+'\t\t'+str(Alg1RMaxRank)+'\t\t'+str(Alg1CostMaxRank)+'\t\t'+str(Alg1ProbeCost)+'\t\t'+str(Alg1MaxRank_Identi_link)+'\t\t'+str(MinMonMaxRank)+'\t\t'+str(CostMinMonMaxRank)+'\t\t'+str(RMinMonMaxRank)+'\t\t'+str(MinMonMaxRank_Identi_link)+'\t\t'+str(Nodes)+'\t\t'+str(Edges)+'\n'
        file.write(raw_line)
        file.close()
"""
def write_stat_monitors(path_to_stats,filename_stat,seed_random,alfa,
...
def write_stat_tomo(path_to_stats,filename_stat,prob_edge,seed_random,alfa,
        ...
        if not os.path.exists(path_to_file_stat):
            #print 'non esiste lo creo'
            file=open(path_to_file_stat,'w+')
            name_of_colunms="Prob_Edge\tSeed\t\tAlfa\tOPT_Nodes\tOPT_Edges\tTotal_OPT\tTotal_OPT_sol_nodes\tTotal_OPT_sol_edges\tTotal_OPT_sol\tOPT_gray_Nodes\tOPT_gray_edges\tTotal_OPT_gray\tIBB_nodes\tIBB_edges\tIBB_total\tIBB_truely_nodes\tIBB_truely_edges\tIBB_truely_total\tFLOW_FIXD\tNUM_COUPL\tVAR_DISTR\tERROR_FLG\t\tDirImages\n"
        ...
        tot_rip_IBB=(num_rip_IBB_optimal_nodes+num_rip_IBB_optimal_edges)
        tot_rip_truely_IBB=(num_rip_IBB_truely_optimal_nodes+num_rip_IBB_truely_optimal_edges)
        ...
        print 'tot_IBB %d = %d + %d'%(tot_rip_IBB, num_rip_IBB_optimal_nodes, num_rip_IBB_optimal_edges)
        print 'tot_IBB_truely %d = %d + %d'%(tot_rip_truely_IBB, num_rip_IBB_truely_optimal_nodes, num_rip_IBB_truely_optimal_edges)
        print 'tot_opt_in_sol %d = %d + %d'%(tot_rip_opt_total, num_rip_optimal_total_nodes,num_rip_optimal_total_edges)

        error='-'

        file=open(path_to_file_stat,'a')
        raw_line=str(prob_edge)+'\t\t'+str(seed_random)+'\t\t'+str(alfa)+'\t\t'+str(num_rip_optimal_nodes)+'\t\t'+str(num_rip_optimal_edges)+'\t\t'+str(tot_rip_opt)+'\t\t'+str(num_rip_optimal_total_nodes)+'\t\t'+str(num_rip_optimal_total_edges)+'\t\t'+str(tot_rip_opt_total)+'\t\t'+str(num_rip_optimal_gray_nodes)+'\t\t'+str(num_rip_optimal_gray_edges)+'\t\t'+str(tot_rip_opt_gray)+'\t\t'+str(num_rip_IBB_optimal_nodes)+'\t\t'+str(num_rip_IBB_optimal_edges)+'\t\t'+str(tot_rip_IBB)+'\t\t'+str(num_rip_IBB_truely_optimal_nodes)+'\t\t'+str(num_rip_IBB_truely_optimal_edges)+'\t\t'+str(tot_rip_truely_IBB)+'\t\t'+str(flow_c_value)+'\t\t'+str(number_of_couple)+'\t\t'+str(var_distruption)+'\t\t'+error+'\t\t'+'Simulazione_'+str(num_sim)+'\n'
        file.write(raw_line)
        file.close()
...
def write_stat_num_reparation(path_to_stats,filename_stat,prob_edge,seed_random,alfa,
        ...
        tot_rip_BB_exp_opt=(num_rip_BB_expected_optimal_nodes+num_rip_BB_expected_optimal_edges)
        tot_truely_BB_exp_opt=(num_rip_BB_expected_truely_optimal_nodes+ num_rip_BB_expected_truely_optimal_edges)
        ...
        print 'tot_BB_expected_opt %d = %d + %d'%(tot_rip_BB_exp_opt, num_rip_BB_expected_optimal_nodes,num_rip_BB_expected_optimal_edges)
        print 'tot_BB_exp_opt_truely %d = %d + %d'%(tot_truely_BB_exp_opt,num_rip_BB_expected_truely_optimal_nodes ,num_rip_BB_expected_truely_optimal_edges)
                ...
                for elem in range(0,len(sorted_array_of_ratio),1):
                    if (bc_found==False):
                        ...
                        if result_check==True:
                            ...
                            result_routability_check=check_routability(get_graph_from_destroyed_graph(graph_splitted),temp_green_edges_after_split)
                        else:
...
def split_by_capacity_path_and_ranking_max_split(H,counter_isp,distance_metric,nodes_recovered,type_of_bet,always_split):
    ...
    array_sorted_bcs=[]
    array_sorted_bcs=get_bc(H)
    #q,w,e=select_betweeness(H,green_edges,distance_metric,type_of_bet)
    ...
    for path in paths_for_pruning:
        ...
        if flow_reduced!=flow_to_prune:
            ...
            if demand==flow_reduced:
                ...
                green_arc=(id_source,id_target)
                green_arc_reverse=(id_target,id_source)
                if (id_source,id_target) not in edges_removed:
                    edges_removed.append((id_source,id_target))
                """
                if green_arc in all_graph_paths:
                    del all_graph_paths[green_arc]
                elif green_arc_reverse in all_graph_paths:
...
def pruning_one_hop(H,recovered_edges_one_hop,distance_metric,counter_isp,type_of_bet):
    ...
    return couple_pruned_one_hop,edges_removed
...
def add_edges_recovered_to_graph(H,graph_built,edges_recovered):
    ...
    for edge in edges_recovered:
        ...
        if source not in graph_built.nodes():
            status=H.node[source]['status']
            long=H.node[source]['Longitude']
            lat=H.node[source]['Latitude']
            graph_built.add_node(H.node[source]['id'],status=status,true_status=H.node[node]['true_status'],id=source,Longitude=long,Latitude=lat)
        if target not in graph_built.nodes():
            status=H.node[target]['status']
            long=H.node[target]['Longitude']
            lat=H.node[target]['Latitude']
            graph_built.add_node(H.node[target]['id'],status=status,true_status=H.node[node]['true_status'],id=target,Longitude=long,Latitude=lat)
        ...
        for node in H.nodes():
            if H.node[node]['id']==id_nodo:
                ...
                if id_nodo not in graph_built.nodes():
                    ...
                    graph_built.add_node(H.node[node]['id'],status=H.node[node]['status'],true_status=H.node[node]['true_status'],id=id_nodo,Longitude=long,Latitude=lat)
                
        #supply graph senza archi verdi
        only_supply=get_supply_graph(H,get_green_edges(H))
        neighbors=nx.neighbors(only_supply,id_nodo)
        for neigh in neighbors:
            if only_supply.node[neigh]['status']!='destroyed':
                keydict=only_supply[id_nodo][neigh]
                for k in keydict:
                    if only_supply[id_nodo][neigh][k]['type']=='normal':
                        if only_supply[id_nodo][neigh][k]['status']=='on':
                            ...
                            if not graph_built.has_edge(id_nodo,neigh):
                                ...
                                flag_edge=False
                                #counter_k=-1
                                keydict_new=graph_built[id_nodo][neigh]
                                for k_new in keydict_new:
                                    #counter_k+=1
...
def Find_truely_recovered_edges(H,edges_recovered_isp, edges_really_dest):
...
#				    edges_truely_recovered.append(edge)
    for edge in edges_recovered_isp:
                ...
                for k in keydict:
                    arc=(edge[0],edge[1])
                    arc_reverse=(edge[1],edge[0])
                    if arc in edges_really_dest or arc_reverse in edges_really_dest:
...
def assign_random_capacity_to_edges(H,filename):
    ...
    for edge in H.edges():
        random_cap=random.randint(60,100) #(20,50) bood
        #random_cap=1000
...
def init_probe_nodes(green_edges):
    owned_nodes = []
            ...
            for k in keydict:
                ...
                if G[id_source][id_target][k]['status'] == 'on' or G[id_source][id_target][k]['status'] == 'repaired':
                    if G.node[id_target]['status']=='on':
                        ...
                        if (id_target not in probe_nodes) and (id_target not in discovered_nodes):
                        ...
                        if H.node[id_target]['true_status'] == 'on':
                            ...
                            if (id_target not in probe_nodes) and (id_target not in destroyed_nodes):
                                destroyed_nodes.append(id_target)			
    return discovered_nodes,destroyed_nodes
...
def add_edges_recovered_to_graph_gray(H,graph_built,edges_recovered):
    #print 'adsfjhadslkfjasdlkfjcsaldkfv seklfjhsdlfjsdlfkjsdalkfajsdlkfjasdl kfjasdlkfajsdl fjadsalfk aodsfj'
...
def recover_gray(H,nodes_repaired,edge_repaired, owned_nodes, edges_recovered, hops):
    ...
    information_gain(H, H, owned_nodes, edges_recovered, hops)
...
def probe_network(H, G, nodes, edges_destroyed):
    ...
    for node in nodes:
        ...
        for edge in edges:
            id_source=edge[0]
            id_target=edge[1]
            if (id_source,id_target) in edges_destroyed or (id_target,id_source) in edges_destroyed:
                continue
            keydict=H[id_source][id_target]
            for k in keydict:
                if H[id_source][id_target][k]['color'] == 'gray' and H[id_source][id_target][k]['true_status'] == 'on' and H.node[id_target]['color'] == 'gray' and H.node[id_target]['true_status'] == 'on':
                    ...
                    if G.has_edge(id_source,id_target):
                        ...
                        for k2 in keydict2:
                            print G[id_source][id_target]
                            if G[id_source][id_target][k2]['type'] == 'normal':
