...
def optimal_SDN_LP(H,green_edges, K, demand_flows, w_l, w_h, Thrh, Thr, weights):
    ...
    Max_Time,my_used_arc, my_delta, Total_Hop, Total_ReRouted, MaxCong, Satisfied_Demand = optimize_SDN(H,nodes,demand_flows,green_edges,arcs,capacity,K,inflow, demand_value, w_l, w_h, Thrh, Thr, weights)
    ...
    """
    return Objective, my_delta, H, my_used_arc, Max_Time, Total_Hop, Total_ReRouted, MaxCong, Satisfied_Demand #Deltah, Thetah, H
...
def optimize_SDN(H,nodes,demand_flows,green_edges,arcs,capacity,K,inflow, demand_value, w_l, w_h, Thrh, Thr, weights):
    ...
    i = 0
    for h in demand_flows:
        BigDeltah[h] = m.addVar(ub=len(demand_flows), obj=w_h, vtype=GRB.CONTINUOUS, name='BigDeltah_%s' % (h)) # 2.0
    ...
    m.update()
    for h in demand_flows:
        m.addConstr(BigDeltah[h] <= Deltah[h]*len(demand_flows), 'Linear1_%s' % (h))
        m.addConstr(BigDeltah[h] >= 0, 'Linear2_%s' % (h))
        m.addConstr(BigDeltah[h] <= T[1], 'Linear3_%s' % (h))
        m.addConstr(BigDeltah[h] >= (T[1] - len(demand_flows)*(1-Deltah[h]) ), 'Linear4_%s' % (h))
    m.update()
    print 'Demand Value:'
    print demand_value
    ...
    if m.status == GRB.status.OPTIMAL:
            ...
            if (ranm >= minbound) and (ranm <= (minbound +x )):
              ...
              for i in range(0, length_selected -1):
                ...
                if (Sel_path[i], Sel_path[i+1]) in H.edges():
                    ...
                    new_cap = new_cap + demand_value[h]/6
                    Satisfied_Demand = demand_value[h]/6
                    #Temp_Spare[h] = capacity[Sel_path[i],Sel_path[i+1]]-(Usedcapacity[Sel_path[i],Sel_path[i+1]] + Satisfied_Demand)
                    #print 'JJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJ'
                    #print 'Temp_Spare[h], Spare[h],capacity[Sel_path[i],Sel_path[i+1]], Usedcapacity[Sel_path[i],Sel_path[i+1]]'
                    #print Temp_Spare[h], Spare[h],capacity[Sel_path[i],Sel_path[i+1]], Usedcapacity[Sel_path[i],Sel_path[i+1]] 
                    #if(Temp_Spare[h] < Spare[h]):
                    #    Spare[h] = Temp_Spare[h]
                    #if(Temp_Spare[h] <0):
                    #    Spare[h] = 0 
                ...
                if (Sel_path[i+1], Sel_path[i]) in H.edges():
                    ...
                    new_cap = new_cap + demand_value[h]/6
                    Satisfied_Demand = demand_value[h]/6
                    #Temp_Spare[h] = capacity[Sel_path[i+1],Sel_path[i]]- (Usedcapacity[Sel_path[i+1],Sel_path[i]] + Satisfied_Demand)
                    #if(Temp_Spare[h] < Spare[h]):
                    #    Spare[h] = Temp_Spare[h]
                    #if(Temp_Spare[h] <0):
                    #    Spare[h] =0
        ...
        #  #print Usedcapacity[id_source, id_target]
        ...
        #Compute the satisfied demand:
        Total_flows = 0
        Satisfied_Demand = 0
        for h in demand_flows:
            #Compute the spare capacities every time
            this_satisfied = 0
            Spare[h] = demand_value[h]
            for e in H.edges():
              i = e[0]
              j = e[1]
              if(RR_used_arc[h,i,j]==1):
                  print 'JJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJ'
                  print 'Temp_Spare[h], Spare[h],capacity[i,j], Usedcapacity[i,j]'
                  Temp_Spare[h] = capacity[i,j] - Usedcapacity[i,j]
                  print Temp_Spare[h], Spare[h],capacity[i,j], Usedcapacity[i,j]
                  if(Temp_Spare[h] <Spare[h]):
                      Spare[h] = Temp_Spare[h]
                  if(Temp_Spare[h] <0):
                      Spare[h] = 0
            ####Up to Here 
            Total_flows= Total_flows + demand_value[h]
            Satisfied_Demand = Satisfied_Demand+ demand_value[h]/6
            this_satisfied= demand_value[h]/6
            print 'HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH'
            print Spare[h], demand_value[h], Satisfied_Demand
            if((Spare[h] >0) and (Spare[h] < demand_value[h])):
                Satisfied_Demand = Satisfied_Demand + Spare[h] 
                this_satisfied= demand_value[h]/6+Spare[h]#this_satisfied+Spare[h]
            if((Spare[h] >0) and (Spare[h] >= demand_value[h])):
                Satisfied_Demand = Satisfied_Demand+ (5*demand_value[h])/6
                this_satisfied = demand_value[h]#this_satisfied+(5*demand_value[h])/6
            #Update capacities
            #new_cap = 0
            for e in H.edges():
                new_cap = 0
                ...
                j = e[1]
                if(RR_used_arc[h,i,j]==1):
                    new_cap = Usedcapacity[i,j]+ (this_satisfied - demand_value[h]/6)
                    Usedcapacity.update({(i,j):new_cap})
                    print 'GGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG'
                    print Usedcapacity[i, j], i,j, new_cap
 
        Satisfied_Demand = Satisfied_Demand/Total_flows
        #Compute the maximum amount of congestion: That is for all i,j \in E Usedcapacity < MaxCong
        MaxCong = 0
        for edge in H.edges():
          id_source=edge[0]
        ...
        for h in demand_flows:
            for i,j in arcs:
                if (RR_used_arc[h,i,j]==1): #or RR_used_arc[h,j,i]==1):
                    RR_Total_Hop = RR_Total_Hop + 1
                if (RR_used_arc[h,j,i]==1):
                    RR_Total_Hop = RR_Total_Hop + 1
        ...
        print 'MaxCongestion'
        print MaxCong
        return Max_Time_RR,RR_used_arc, my_delta, RR_Total_Hop, Total_ReRouted, MaxCong, Satisfied_Demand #my_used_arc -> RR_used_arc
