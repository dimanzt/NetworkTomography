...
def optimal_SDN_LP(H,green_edges, K, demand_flows, w_l, w_h, Thrh, Thr):
    ...
    for edge in green_edges:
        ...
        if not demand_loss.has_key(flow_label):
            demand_loss.update({flow_label:0})
        for node in nodes:
            flow_value=0
            flow_b_value=0
    ...
    my_altered_switches,my_used_arc, my_delta, MaxCong =optimize_SDN(H,nodes,demand_flows,green_edges,arcs,capacity,K,inflow, demand_value, demand_loss, w_l, w_h, Thrh, Thr)
    ...
    """
    return Objective, my_delta, H, my_used_arc, MaxCong #Deltah, Thetah, H



def optimize_SDN(H,nodes,demand_flows,green_edges,arcs,capacity,K,inflow, demand_value,demand_loss, w_l, w_h, Thrh, Thr):
    ...
    #Thrh= len(arcs)/2
    for h in demand_flows:
        m.addConstr((quicksum(x[h,i,j]*K[h,i,j] for i,j in arcs) <= Thrh), 'Qosh_%s' % (h))
    m.update()
    #Thr= Thrh*len(demand_flows)*0.8
    m.addConstr((quicksum(x[h,i,j]*K[h,i,j] for h in demand_flows for i,j in arcs) <= Thr), 'TotalQos_%s' % (h))
    m.update()
    ...
    if m.status == GRB.status.OPTIMAL:
        ...
        for h in demand_flows:
          ...
          for p in paths:
            ...
            for i in range(0, length_path-1):
               if (p[i],p[i+1]) in H.edges():
                  if ((capacity[p[i],p[i+1]] - Usedcapacity[p[i],p[i+1]]) < Min_Capacity):
                      Min_Capacity = capacity[p[i],p[i+1]]
               if (p[i+1],p[i]) in H.edges():
                  if ((capacity[p[i+1],p[i]] - Usedcapacity[p[i+1],p[i]]) < Min_Capacity):
          ...
          for x in path_probabilities:
            if (ranm >= minbound) and (ranm <= (minbound +x )):
              ...
              for i in range(0, length_selected -1):
                ...
                if (Sel_path[i], Sel_path[i+1]) in H.edges():
                    ...
                    if (new_cap <= capacity[Sel_path[i],Sel_path[i+1]]):
                      RR_used_arc[h,Sel_path[i],Sel_path[i+1]] = 1
                      Usedcapacity.update({(Sel_path[i],Sel_path[i+1]):new_cap})
                      print'Used Capacity:'
                      print Usedcapacity[Sel_path[i],Sel_path[i+1]]
                    else:
                      demand_loss.update({h:1})
                if (Sel_path[i+1], Sel_path[i]) in H.edges():
                    new_cap = Usedcapacity[Sel_path[i+1],Sel_path[i]]
                    new_cap = new_cap + demand_value[h]
                    if (new_cap <= capacity[Sel_path[i+1],Sel_path[i]]):
                      RR_used_arc[h,Sel_path[i+1],Sel_path[i]] = 1
            ...
            cnt = cnt+1
        ######################################################################
        #Compute Total demand Loss:
        Total_demand_loss = 0
        for h in demand_flows:
          if (demand_loss[h]==1):
            Total_demand_loss = Total_demand_loss +1
        ...
        print 'MaxCongestion'
        print MaxCong
        return my_altered_switch,RR_used_arc, my_delta, Total_demand_loss #my_used_arc -> RR_used_arc
