...
def optimal_SDN_LP(H,green_edges, K, demand_flows, w_l, w_h, Thrh, Thr):
    ...
    my_altered_switches,my_used_arc, my_delta, MaxCong =optimize_SDN(H,nodes,demand_flows,green_edges,arcs,capacity,K,inflow, demand_value, w_l, w_h, Thrh, Thr)
...
def optimize_SDN(H,nodes,demand_flows,green_edges,arcs,capacity,K,inflow, demand_value, w_l, w_h, Thrh, Thr):
    ...
    m.update()
    T = {}
    T= m.addVar(ub=len(demand_flows), obj=0.0, vtype=GRB.BINARY, name='StragglerT')
    """
    t = {}
    for i in nodes:
    ...
    #Thrh= len(arcs)/2
    for h in demand_flows:
        m.addConstr((quicksum(x[h,i,j] for i,j in arcs) <= Thrh), 'Qosh_%s' % (h))
    m.update()
    #Thr= Thrh*len(demand_flows)*0.8
    m.addConstr((quicksum(x[h,i,j] for h in demand_flows for i,j in arcs) <= Thr), 'TotalQos_%s' % (h))
    m.update()
    ...
    for i in nodes:
        for h in demand_flows:
            m.addConstr(t[i,h] <= ((quicksum(x[h,i,j]+K[h,i,j] -2*x[h,i,j]*K[h,i,j] for j in H.neighbors(i)))) , 'AlteredSwitch_%s_%s' % (i,h))
    m.update()
    for i in nodes:
        m.addConstr((quicksum(t[i,h] for h in demand_flows)) <= T, 'StragglerConst_%s' % (i))
    m.update()
    ...
    if m.status == GRB.status.OPTIMAL:
        ...
        print 'MaxCongestion'
        print MaxCong
        return my_altered_switch,RR_used_arc, my_delta, MaxCong #my_used_arc -> RR_used_arc
